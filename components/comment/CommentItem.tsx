...
export default function CommentItem({
    ...
    const visibleReplies = showAllReplies ? repliesList : repliesList.slice(0, 3);
    const hiddenRepliesCount = repliesList.length - 3;

    const formatTime = (date: string) => {
        return formatDistanceToNow(new Date(date), { addSuffix: true, locale: vi });
    };

    // Click outside để đóng reaction picker
    useEffect(() => {
    ...
    // Click outside để đóng more menu
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (
                moreMenuRef.current &&
    ...
    return (
        <>
            <div className={`flex gap-3 ${!isReply ? 'mb-4' : 'mb-3'}`} style={{ marginLeft: isReply ? `${Math.min(depth * 20, 40)}px` : 0 }}>
                {/* Avatar */}
                ...
                <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mb-1 flex-wrap gap-2">
                            ...
                            <div className="relative">
                                ...
                                {showMoreMenu && (
                                    ...
                                    >
                                        {/* Chỉ hiển thị báo cáo nếu KHÔNG phải chủ sở hữu */}
                                        {!isOwner && (
                                            <button
                    ...
                    {!comment.isDeleted && (
                        <div className="flex items-center gap-3 mt-1 ml-1 relative">
                            {/* Reaction Picker - nằm PHÍA TRÊN nút like */}
                            {showReactionPicker && (
                                <div className="absolute bottom-8 left-0 z-10 bg-white rounded-full shadow-lg border border-gray-200 flex p-1 gap-0.5">
                            ...
                            <div className="relative">
                                ...
                                >
                                    ...
                                    {reactionCount > 0 && <span className="text-gray-500 ml-0.5">{reactionCount}</span>}
                                </button>
                            </div>
                    ...
                    {hasReplies && (
                        <div className="mt-3 space-y-3">
                            ...
                            {repliesList.length > 3 && showAllReplies && (
                                <button
            ...
            {showReportModal && (
                <div
                    ...
                    style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: -20, zIndex: 9999 }}
                >
                    <div
                        className="bg-white rounded-xl w-full max-w-md max-h-[70vh] overflow-y-auto shadow-xl relative"
                    ...
                    >
                        ...
                        <div className="p-5 space-y-4">
                            <p className="text-sm text-gray-600">
                                Bình luận từ <span className="font-semibold">{getUserName()}</span>
                            </p>
                        ...
                        <div className="flex gap-3 p-5 pt-0 border-t border-gray-100 mt-2">
                            <button
                                onClick={() => setShowReportModal(false)}
