...
export default function EditPostModal({ post, isOpen, onClose, onPostUpdated }: EditPostModalProps) {
    ...
    const [privacy, setPrivacy] = useState<'public' | 'friends' | 'private'>(post.privacy);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!token) {
            toast.error('Bạn cần đăng nhập để cập nhật bài viết');
            return;
        }
        ...
        try {
            const result = await forumApi.updatePost(
                post._id,
                {
                    content,
                    images,
