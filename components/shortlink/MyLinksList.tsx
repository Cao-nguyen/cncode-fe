...
export function MyLinksList() {
    ...
    if (links.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-12 gap-3 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl">
                <Link21 size={32} variant="Outline" className="text-gray-300 dark:text-gray-600" />
                <div className="text-center">
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Chưa có link nào</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                        Hãy tạo link rút gọn đầu tiên của bạn!
                    </p>
                </div>
            </div>
        );
...
function Pagination({
    ...
    return (
        <div className="flex items-center justify-center gap-2 pt-2">
            ...
            <span className="px-3 py-1.5 text-sm text-gray-600 dark:text-gray-400 font-medium">
                {current} / {total}
            </span>
            <button
                onClick={() => onChange(current + 1)}
